...
import random
from ged_util_nas301 import nas301_to_digraph, ged_nas301, ged_nas301_optimize, crossover_edit_path_center_nas301, digraph_to_nas301, edge_match
...
with open(result_file_name, 'rb') as result_file:
    ...
    #random_data, evolution_mutation_data, evolution_crossover_raw_data,  evolution_crossover_refined_data = pickle.load(result_file)
...
plt.plot(optimal_ratio_list, label='random')
optimal_ratio_list = calc_optimal_ratio(evolution_mutation_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, label='mutation')
optimal_ratio_list = calc_optimal_ratio(evolution_crossover_raw_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, label='raw_crossover')
optimal_ratio_list = calc_optimal_ratio(evolution_crossover_match_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, label='match_crossover')
optimal_ratio_list = calc_optimal_ratio(evolution_crossover_refined_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, label='refined_crossover')
plt.ylabel("optimal ratio")
plt.xlabel("timestep")
plt.title("percentage of runs that reaches optimal solution ({})".format(DATASET_NAME))
plt.legend()
plt.show()
'''
'''
...
#plt.ylim(0.92, 0.95)
plt.title('Convergence Plot on NAS-bench-301 (30 runs)', fontweight="bold")
plt.xlabel('total number of architectures evaluated')
plt.ylabel('test accuracy')
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','test_accuracy_center_crossover_pop{}_tournament{}_run{}_evaluation{}_icml.pdf'.format(
                                                population_size, tournament_size, repeat_num, max_time_budget))
f.savefig(plot_file_name, bbox_inches='tight')
