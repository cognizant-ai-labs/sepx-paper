...
#benchmark_name = "nasnlp"
#benchmark_name = "nasnlp_GEDfitness_mp"
#benchmark_name = "nasnlp_GEDfitness_LSTM_mp"
...
with open(result_file_name, 'rb') as result_file:
    ...
    _, _, _, evolution_crossover_refined_data = pickle.load(result_file)
...
total_count = 0
for _, _, _, _, _, _, _, theory_list in evolution_crossover_refined_data:
    for time_spent, crossover_ged, ged_to_best_1, ged_to_best_2, ged_parents_edge_only, ged_to_best_1_edge_only, ged_to_best_2_edge_only, num_edges_1, num_edges_2 in theory_list:
...
ax = sns.heatmap(heatmap_graph_crossover_empirical[:16,117:]/total_count, fmt='.3f', annot=True, linewidth=0.5, xticklabels=list(reversed(range(16))))
ax.invert_yaxis()
ax.invert_xaxis()
plt.title("Relative Frequency of Events Happened (NAS-bench-NLP)", fontweight="bold")
plt.ylabel("$d_e^*$ between parent 1 and 2")
plt.xlabel("$d_e^*$ between parent 1 and global optimum")
plt.xticks(rotation=90)
plt.yticks(rotation=0)

plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','num_occurances_GED_parent_nc_parent2_n_{}_heatmap.pdf'.format(n))
...
ax = sns.heatmap(heatmap_num_edges_empirical[7:17,7:17]/total_count, fmt='.3f', annot=True, linewidth=0.5, yticklabels=list(range(7,17)), xticklabels=list(range(7,17)))
ax.invert_yaxis()
plt.title("Relative Frequency of Events Happened (NAS-bench-NLP)", fontweight="bold")
...
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','num_occurances_num_edges_n_{}_heatmap.pdf'.format(n))
fig.savefig(plot_file_name, bbox_inches='tight')
plt.show()
'''
