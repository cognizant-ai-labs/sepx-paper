...
for nd_12 in range(1,21):#range(n+1):
    for nc_1 in range(112,n+1):#range(n+1):
...
ax = sns.heatmap(heatmap_graph_crossover[:21,112:], annot=True, fmt='.2f', linewidth=0.5, xticklabels=list(reversed(range(21))))
ax.invert_yaxis()
ax.invert_xaxis()
...
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','random_vs_mutation_pm_{}_alpha_new_difference_GED_2-to-optimal_n_{}_n10_{}_n12_{}_heatmap_icml.pdf'.format(pm, n, n1_0, n1_2))
