...
for i in range(n_iters):
    ...
    for run in range(run_num):
        ...
        valid_acc_tmp_list.append(1-y_star_valid-rl_data[run]['regret_validation'][i])
...
plt.plot(xs, mean_rl, color='yellow', label='RL', linewidth=2)
...
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','validation_accuracy_rl_lr_{}_pop{}_tournament{}_run{}_evaluation{}.pdf'.format(
                                                lr, population_size, tournament_size, repeat_num, max_time_budget))
f.savefig(plot_file_name, bbox_inches='tight')
