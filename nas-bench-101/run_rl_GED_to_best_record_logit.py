...
class REINFORCEOptimizer(object):
    ...
    def step(self):
        ...
        #print([logit_cur.numpy() for logit_cur in self._logits])
        self.logits_record.append([logit_cur.numpy() for logit_cur in self._logits])
        attempts = 0
        while True:
...
parser.add_argument('--n_runs', default=1, type=int, nargs='?', help='total number of runs')
...
for run in range(num_run):
    ...
    res["logits_record"] = optimizer.logits_record

    print(res['regret_test'][-1])
    result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','nas101_rl_GED_fitness_steps_{}_lr_{}_run_{}.pkl'.format(
                                                    args.n_iters, args.lr, run))
    with open(result_file_name, 'wb') as result_file:
        pickle.dump(res, result_file)
    ...
    '''
'''
result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','nas101_rl_steps_{}_lr_{}_run_{}.pkl'.format(
                                                args.n_iters, args.lr, num_run))
with open(result_file_name, 'wb') as result_file:
    pickle.dump(results_list, result_file)
'''
