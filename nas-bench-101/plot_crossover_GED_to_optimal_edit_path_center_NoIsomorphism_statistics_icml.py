...
for run in range(run_num):
    result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results_rl','nas101_rl_GED_fitness_steps_{}_lr_{}_run_{}.pkl'.format(
                                                    n_iters, lr, run))
    with open(result_file_name, 'rb') as result_file:
        rl_data.append(pickle.load(result_file))
    #print(rl_data[-1]['logits_record'][-1])
print(rl_data[0].keys())
mean_rl = []
std_rl = []
mean_GED_rl = []
std_GED_rl = []
for i in range(n_iters):
    ...
    for run in range(run_num):
        ...
        GED_tmp_list.append(rl_data[run]['regret_validation'][i])
    mean_rl.append(np.mean(test_acc_tmp_list))
    std_rl.append(np.std(test_acc_tmp_list))
    mean_GED_rl.append(np.mean(GED_tmp_list))
    std_GED_rl.append(np.std(GED_tmp_list))
...
def plot_data_ged_to_best(data, color, label, marker, gran=1, max_budget=5000000):
    """Computes the mean and IQR fixed time steps."""
    xs = range(0, max_budget+1, gran)
    mean = [0.0]
    per25 = [0.0]
...
plt.plot(xs, mean_rl[:max_time_budget], color='#ff7f0e', label='RL', marker='s', markevery=0.2, markersize=10, linewidth=2)
plt.fill_between(xs, np.array(mean_rl[:max_time_budget])+np.array(std_rl[:max_time_budget]), np.array(mean_rl[:max_time_budget])-np.array(std_rl[:max_time_budget]), alpha=0.1, linewidth=0, facecolor='#ff7f0e')
...
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','test_accuracy_rl_GED_fitness_lr_{}_pop{}_tournament{}_run{}_evaluation{}.pdf'.format(
...
f = plt.figure()
plot_data_ged_to_best(random_data, 'c', 'random search', 'o', max_budget=max_time_budget)
plot_data_ged_to_best(evolution_mutation_data, 'blue', 'RE mutation only', 'v', max_budget=max_time_budget)
plot_data_ged_to_best(evolution_crossover_raw_data, 'g', 'RE + standard crossover', '^', max_budget=max_time_budget)
xs = range(1, max_time_budget+1)
plt.plot(xs, mean_GED_rl[:max_time_budget], color='#ff7f0e', label='RL', marker='s', markevery=0.2, markersize=10, linewidth=2)
plt.fill_between(xs, np.array(mean_GED_rl[:max_time_budget])+np.array(std_GED_rl[:max_time_budget]), np.array(mean_GED_rl[:max_time_budget])-np.array(std_GED_rl[:max_time_budget]), alpha=0.1, linewidth=0, facecolor='#ff7f0e')
...
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','ged_to_best_rl_lr_{}_pop{}_tournament{}_run{}_evaluation{}_icml.pdf'.format(
                                                lr, population_size, tournament_size, repeat_num, max_time_budget))
f.savefig(plot_file_name, bbox_inches='tight')
