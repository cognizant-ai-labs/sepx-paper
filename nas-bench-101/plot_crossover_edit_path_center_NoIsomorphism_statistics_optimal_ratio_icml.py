...
exp_variant = "edit_path_center_crossover_NoIsomorphism_stats"
...
result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_pop{}_tournament{}_run{}_evaluation{}.pkl'.format(
                                                exp_variant, population_size, tournament_size, repeat_num, max_time_budget))
...
y_star_valid = 0.04944576819737756
...
for i in range(n_iters):
    ...
    for run in range(run_num):
        ...
        valid_acc_tmp = 1-y_star_valid-rl_data[run]['regret_validation'][i]
        valid_acc_tmp_list.append(valid_acc_tmp)
        if abs(valid_acc_tmp-optimal_target) <= 0.0000001:
            count+=1
    mean_rl.append(np.mean(valid_acc_tmp_list))
    median_rl.append(np.median(valid_acc_tmp_list))
    std_rl.append(np.std(valid_acc_tmp_list))

    optimal_ratio_rl.append(count/run_num)
...
plt.plot(optimal_ratio_list, color='c', marker='o', markevery=0.2, markersize=10, label='random search')
optimal_ratio_list = calc_optimal_ratio(evolution_mutation_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, color='blue', marker='v', markevery=0.2, markersize=10, label='RE mutation only')
optimal_ratio_list = calc_optimal_ratio(evolution_crossover_raw_data, data_index, optimal_target)
plt.plot(optimal_ratio_list, color='g', marker='^', markevery=0.2, markersize=10, label='RE + standard crossover')
optimal_ratio_list = calc_optimal_ratio(evolution_crossover_refined_data, data_index, optimal_target)
plt.plot(optimal_ratio_rl, color='#ff7f0e', marker='s', markevery=0.2, markersize=10, label='RL')
plt.plot(optimal_ratio_list, color='red', marker='*', markevery=0.2, markersize=10, label='RE + SEP crossover')
...
plt.legend(fontsize=12, loc='lower left', bbox_to_anchor=(0.25, 0.52))
plt.grid()
plt.show()

plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','optimal_ratio_center_crossover_rl_pop{}_tournament{}_run{}_evaluation{}_icml.pdf'.format(
...
def plot_data_val(data, color, label, marker, gran=1, max_budget=5000000):
    """Computes the mean and IQR fixed time steps."""
    xs = range(0, max_budget+1, gran)
    mean = [0.0]
    per25 = [0.0]
...
'''
f = plt.figure()
plot_data(random_data, 'c', 'random search', max_budget=max_time_budget)
plot_data(evolution_mutation_data, 'blue', 'RE mutation only', max_budget=max_time_budget)
...
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','test_accuracy_rl_lr_{}_pop{}_tournament{}_run{}_evaluation{}.pdf'.format(
                                                lr, population_size, tournament_size, repeat_num, max_time_budget))
f.savefig(plot_file_name, bbox_inches='tight')
'''
f = plt.figure()
plot_data_val(random_data, 'c', 'random search', 'o', max_budget=max_time_budget)
plot_data_val(evolution_mutation_data, 'blue', 'RE mutation only', 'v', max_budget=max_time_budget)
plot_data_val(evolution_crossover_raw_data, 'g', 'RE + standard crossover', '^', max_budget=max_time_budget)
...
plt.plot(xs, mean_rl, color='#ff7f0e', marker='s', markevery=0.2, markersize=10, label='RL', linewidth=2)
#plt.plot(xs, median_rl, color='yellow', label='RL', linewidth=2)
plt.fill_between(xs, np.array(mean_rl)+np.array(std_rl), np.array(mean_rl)-np.array(std_rl), alpha=0.1, linewidth=0, facecolor='yellow')
plot_data_val(evolution_crossover_refined_data, 'red', 'RE + SEP crossover', '*', max_budget=max_time_budget)
...
plt.grid()
plt.show()
plot_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Plots','validation_accuracy_rl_lr_{}_pop{}_tournament{}_run{}_evaluation{}_icml.pdf'.format(
                                                lr, population_size, tournament_size, repeat_num, max_time_budget))
f.savefig(plot_file_name, bbox_inches='tight')
